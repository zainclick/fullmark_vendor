		...
		<div class="col-12 px-0">
			<div class="col-12 p-0 row">
				<div class="col-12 col-lg-4 py-3 px-3">
					...
					{{__('admin/invoices/index.title')}}
				</div>
			
				<div class="col-12 col-lg-8 p-2 text-lg-end">

				<a href="{{route('admin.invoices.create')}}">
					<span class="btn btn-danger export-pdf"> <span class="fas fa-file"></span> {{__('main.export_pdf')}} </span>
				</a>

				<a href="{{route('admin.invoices.import.excel')}}">
					<span class="btn btn-primary import-excel"> <span class="fas fa-file"></span> {{__('main.import_excel')}} </span>
				</a>

				<a href="{{route('admin.invoices.export.excel')}}">
					<span class="btn btn-dark export-excel"> <span class="fas fa-file"></span> {{__('main.export_excel')}} </span>
				</a>

				<a href="{{route('admin.invoices.create')}}">
					<span class="btn btn-success"><span class="fas fa-plus"></span>{{__('admin/invoices/index.create')}}</span>
				</a>
			...
			<table class="table table-bordered  table-hover text-center">
				<thead>
					<tr>
						...
						<th>{{__('admin/invoices/index.payment_amount')}}</th>
						<th>{{__('admin/invoices/index.remainig_amount')}}</th>
				...
				<tbody>
					...
					<tr>
						...
						<td>{{$invoice->Total}}<span style='color: #198754;font-weight: bold;'> {{$invoice->client->Currency}} </span></td>
						<td>{{$invoice->PaymentAmount}}<span style='color: #198754;font-weight: bold;'> {{$invoice->client->Currency}} </span></td>
						<td>{{$invoice->Total - $invoice->PaymentAmount}}<span style='color: #198754;font-weight: bold;'> {{$invoice->client->Currency}} </span></td>
						...
						@endif
							...
							<li>
							@if(auth()->user()->has_access_to('show',$invoice))
							<a class="dropdown-item" href="{{route('admin.invoice.send.archive',$invoice)}}">
							<span style="color: #d501ef;" class="fas fa-archive "></span> {{__('main.archive')}}
							</a>
							@endif
							</li>
