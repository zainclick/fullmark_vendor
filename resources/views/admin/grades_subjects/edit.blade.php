...
<div class="col-12 p-3">
	<div class="col-12 col-lg-12 p-0 ">
		...
		<form id="validate-form" class="row" enctype="multipart/form-data" method="POST" action="{{route('admin.grades_subjects.update',$datasubject)}}">
		@csrf
		@method('PATCH')
		...
		<div class="col-12 col-lg-12 p-0 main-box">
			...
			<div class="col-12 col-lg-3 p-2">
				...
				<select class="form-control" required name="subject" id="subject">
					...
					@foreach($subjects as $subject) 
						<option @if($datasubject->Subject == $subject->id) selected @endif value="{{$subject->id}}">{{$subject->Name}}</option>
					@endforeach
				</select>
			</div>
			</div>
			...
			<div class="col-12 col-lg-3 p-2">
				...
				<select class="form-control" required name="section" id="section">
					...
					@foreach($grades_sections as $section) 
						<option @if($datasubject->Grades_Sections == $section->id) selected @endif value="{{$section->id}}">{{$section->Name}}</option>
					@endforeach
				</select>
			</div>
			</div>
			...
			<div class="col-12 col-lg-3 p-2">
				...
				<select class="form-control" required name="grade" id="grade">
					...
					@foreach($grades as $grade) 
						<option @if($datasubject->Grade == $grade->id) selected @endif value="{{$grade->id}}">{{$grade->Name}}</option>
					@endforeach
				</select>
			</div>
			</div>
			...
			<div class="col-12 col-lg-3 p-2">
				...
				<select class="form-control" required name="status" id="status">
							<option @if($datasubject->Status == 1) selected @endif value="1">{{__('main.active')}}</option>
							<option @if($datasubject->Status == 0) selected @endif value="0">{{__('main.disactive')}}</option>
				</select>				
				</div>
			</div>
